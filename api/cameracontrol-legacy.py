...
class CameraControl:
    ...
    def ffmpeg_open(self):
        ...
        self.ffmpeg = Popen(
            ["ffmpeg", *input_chroma, *input_gphoto, *filters, *ffmpeg_output],
            stdin=PIPE,
        )
    ...
    def chroma_key_image(self, path):
        ...
        Popen(["ffmpeg", *input_chroma, *input_gphoto, *filters, *ffmpeg_output]).wait(
            5
        )
...
def get_running_pid():
    for p in psutil.process_iter(["name", "cmdline"]):
        if (
            p.name() == "python3"
            and p.cmdline()[1].endswith("cameracontrol-legacy.py")
            and p.pid != os.getpid()
        ):
            return p.pid
    return -1
...
def main():
    ...
    python-gphoto2.",
        epilog="If you don't want images to be stored on the camera make sure that capturetarget \
