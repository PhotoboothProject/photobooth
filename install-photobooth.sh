...
# Node.js v18.15.(0 or newer) is needed on installation via git
NEEDS_NODEJS_CHECK=true
NEEDED_NODE_VERSION="v18.15.(0 or newer)"
...
check_nodejs() {
    ...
    if [[ -n "$major" && "$major" -eq "18" ]]; then
        if [[ -n "$minor" && "$minor" -eq "15" ]]; then
...
update_nodejs() {
    ...
    if [ "$RUNNING_ON_PI" = true ]; then
        info "[Package]   Installing Node.js v18.15.0"
        wget -O - https://raw.githubusercontent.com/audstanley/NodeJs-Raspberry-Pi/master/Install-Node.sh | bash
        node-install -v 18.15.0
        NODEJS_CHECKED=true
        check_nodejs
    else
